...
# Store client id and secret in local file export_tf_vars
echo "Saving TF_VARS to Key Vault and export_tf_vars"
export TF_VAR_client_id="$clientId"
export TF_VAR_client_secret="$clientSecret"
# For security reasons, store the client id and secret in azure key vault too
az keyvault secret set --vault-name $KEY_VAULT_NAME --name “TF-VAR-client-id” --value $clientId
az keyvault secret set --vault-name $KEY_VAULT_NAME --name “TF-VAR-client-secret” --value $clientSecret
